...
- 4 个独立面试官风格 Skill：温和教练、架构面试官、压力挑战官、启发导师；每个 Skill 拥有独立的自我介绍邀请语、提示词、评价重点和反馈语气。
...
- 真实面试开场：面试官先邀请候选人自我介绍，再综合简历和介绍中的关键词重排并自然引出技术问题；自我介绍留档但不计分。
- 真实连续面试：每个主问题会按面试官风格和候选人回答继续追问 2–3 轮；大模型不可用时使用风格化本地追问。
- 主问题数量自由输入；当前混合难度下单语言最多 6 题，启用公共基础库后最多 12 题，专项难度最多 2/4 题。
- 分语言持久化知识库与计算机基础公共库；面试实际出题会自动累计并沉淀为可检索 QA，也支持手动新增 QA。
- 学习中心聚合 Go、Java、Spring、Python、C++ 官方/权威 Feed，并提供官方文档、课程、视频和本地学习清单。
- Electron 上下文隔离、动态分配独立本地端口、使用随机访问令牌且仅监听 `127.0.0.1`；简历原文件解析完成后立即删除。
...
开发模式会同时启动 Vite、Electron 和本地 Go 服务。Electron 会为本次进程动态分配一个空闲的 `127.0.0.1` 端口并生成随机访问令牌，避免与旧版进程串线。
